...
\begin{frame}{Regression}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item $X$, $(X_1, \dots, X_K)^\prime$, regressors, predictors, covariates, explanatory variables, features
        \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Regression is a statistical tool that summarizes a relationship between variables in a fixed dataset
...
\begin{itemize}
    ...
    \item $\XX_i$: $K \times 1$ vector of covariates 
    \begin{itemize}
        \item Elements $X_{ki}$
...
\begin{align*}
\XX_i &= \begin{bmatrix} X_{1i} \\ \vdots \\ X_{ki} \\ \vdots \\ X_{Ki} \end{bmatrix} \\
&= \begin{bmatrix}
...
\begin{frame}{Best linear prediction rule (in the population)}
    ...
    \begin{equation*}
        \sum_{j=1}^K \beta_j X_j = \beta^\prime \XX, \quad\text{for } \ \beta = \begin{bmatrix}
        \beta_1, \dots, \beta_K
        \end{bmatrix}^\prime
    \end{equation*}
...
\begin{itemize}
    ...
    \begin{equation*}
        b^\prime X = \sum_{j=1}^K b_j X_j, \quad b = (b_1, \dots, b_K)^\prime
    \end{equation*}
\end{itemize}
...
\begin{equation*}
    \EE [ \epsilon_i \XX_i ] = 0
\end{equation*}
where $\epsilon_i$ is uncorrelated with (\textbf{orthogonal} to) $\XX_i$, leading to the decomposition:
\begin{equation*}
...
\textbf{Thm. 3.1.5: The best linear predictor theorem}: The function $X_i^\prime \beta$ is the best \textbf{linear} predictor of $Y_i$ given $X_i$, in an MSE sense
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Best Linear Predictor to Best Predictor}
    Let $W$ be the ``raw'' regressors, and $X$ be the transformation of regressors:
    \begin{equation*}
        X = T(W) = (T_1 (W), \dots, T_P (W))^\prime
    \end{equation*}
    For example, $Y$ are wages, and $W$ are gender, experience, and education
    \begin{itemize}
        \item $X$ contains the raw regressors AND transformations of raw regressors, like polynomials, logs, and interactions
    \end{itemize}

    \vspace{2ex}
    Constructed regressors allow us to build more flexible and potentially better prediction rules
    \begin{itemize}
        \item Prediction rules $X^\prime \beta = (T(W))^\prime \beta$ may be nonlinear in original regressors
        \item Still linear with respect to parameters $\beta$ and constructed regressors $X$
    \end{itemize}

    \vspace{2ex}
    In the population, \textbf{best predictor} of $Y$ given $W$ is given by the \textbf{conditional expectation function} $g(W)$:
    \begin{equation*}
        g(W) = \EE [Y \vert W]
    \end{equation*}
    Also called the \textbf{regression function of $Y$ on $W$}
\end{frame}
